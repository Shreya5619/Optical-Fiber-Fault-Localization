...
def preprocess_for_model(data_row, debug=False, model=None, scaler=None):
    ...
    try:
        ...
        if(model=='position'):
            return (snr_features+ otdr_features_array).reshape(1, -1)
        if debug:
        ...
        # Apply StandardScaler normalization if scaler is provided
        ...
        if models_status['binary']:
            ...
            if st.button("🔍 Run Binary Classification", key="binary_btn"):
                with st.spinner("Running binary classification..."):
                    ...
                    input_features = preprocess_for_model(st.session_state.input_data, debug=debug_mode, model=st.session_state.binary_model, scaler=st.session_state.scaler)
            ...
            if available_models:
                ...
                if st.button("🔬 Run Detailed Analysis", key="detailed_btn"):
                    with st.spinner("Running detailed analysis..."):
                        input_features = preprocess_for_model(st.session_state.input_data, model=st.session_state.loaded_models.get('class'), scaler=st.session_state.scaler)
                        ...
                        if input_features is not None:
                            ...
                            if models_status['position']:
                                input_features = preprocess_for_model(st.session_state.input_data, model=st.session_state.loaded_models.get('position'), scaler=st.session_state.scaler)
                            ...
                            if models_status['reflectance']:
                                input_features = preprocess_for_model(st.session_state.input_data, model=st.session_state.loaded_models.get('reflectance'), scaler=st.session_state.scaler)
                            ...
                            # Loss Analysis
                            if models_status['loss']:
                                input_features = preprocess_for_model(st.session_state.input_data, model=st.session_state.loaded_models.get('loss'), scaler=st.session_state.scaler)
